...
try:
    ...
    if "hourly" in res:
        ...
        # --- FILTRAGE AUX HEURES SOLAIRES (LEVER A COUCHER) ---
        if "daily" in res and "sunrise" in res["daily"] and "sunset" in res["daily"]:
            sunrises = [pd.to_datetime(s) for s in res["daily"]["sunrise"]]
            sunsets = [pd.to_datetime(s) for s in res["daily"]["sunset"]]

            # Information sunrise/sunset du jour
            sr_str = sunrises[0].strftime("%H:%M")
            ss_str = sunsets[0].strftime("%H:%M")
            st.info(f"🌅 **Lever :** {sr_str}  |  🌇 **Coucher :** {ss_str} *(Filtre VFR de jour actif)*")

            # Masque pour ne garder que les heures entre lever et coucher du soleil
            mask_jour = pd.Series(False, index=df.index)
            for sr, ss in zip(sunrises, sunsets):
                mask_jour |= (df["time"] >= sr) & (df["time"] <= ss)
        ...
        if df_future.empty:
            st.warning("Aucun créneau de jour disponible pour les prochaines heures.")
        else:
            ...
            st.subheader("📊 Prévisions détaillées heure par heure (Heures diurnes uniquement)")
